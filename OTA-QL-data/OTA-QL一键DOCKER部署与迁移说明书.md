...
> 📌 部署脚本托管在公开仓库 [public-data/OTA-QL-data](https://github.com/hhtbing/public-data/tree/main/OTA-QL-data)，由 GitHub Actions 从私有仓库自动同步。
...
```bash
# 子菜单1 设置流程
输入新地址 → 保存到 /opt/ota-ql/.mqtt_addr → 重启容器(可选)
...
# 方式二：手动升级
docker pull ghcr.io/hhtbing/ota-ql:latest
docker stop ota-ql
docker rm ota-ql
# 重新启动容器（使用原有参数）
```
...
```bash
docker pull ghcr.io/hhtbing/ota-ql:v0.2.0
docker stop ota-ql
docker rm ota-ql
# 使用 v0.2.0 标签重新启动
```
...
```bash
# 1. 下载部署脚本
wget -O ota-ql-docker-deploy.sh "https://raw.githubusercontent.com/hhtbing/public-data/main/OTA-QL-data/ota-ql-docker-deploy.sh"
...
# 部署说明书
wget "https://raw.githubusercontent.com/hhtbing/public-data/main/OTA-QL-data/OTA-QL一键DOCKER部署与迁移说明书.md"
```
...
services:
  ota-ql:
    image: ghcr.io/hhtbing/ota-ql:latest
...
反向代理模式持久化保存在 `/opt/ota-ql/.reverse_proxy`，更新部署时自动读取。
