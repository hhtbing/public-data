...
```
执行脚本 → 选择环境(生产/测试) → 设置设备回调地址 → 自动拉取镜像 → 创建数据卷 → 启动容器 → 健康检查 → 显示密码+回调地址
```
...
4. ✅ 检查端口冲突
5. ✅ 拉取最新镜像
6. ✅ 启动容器（5端口映射 + 回调地址环境变量）
7. ✅ 执行健康检查（HTTPS API + 设备网关 双重检测）
8. ✅ 显示管理员初始密码 + 设备回调地址信息
...
> ⚡ **版本历史**: v4.6 (2026-03-08) — 菜单10增加子菜单（设置与查看），支持查看回调地址详情及派生服务地址
