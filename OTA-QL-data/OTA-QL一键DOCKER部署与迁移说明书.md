...
```
执行脚本 → 选择环境(生产/测试) → 设置MQTT服务器地址 → 🆕设置固件下载域名 → 🆕SSL证书配置(交互式+覆盖检查) → 自动拉取镜像 → 创建数据卷 → 启动容器 → 健康检查 → 显示密码+MQTT服务器地址+固件域名
```
...
6. ✅ 检查端口冲突
7. ✅ 拉取最新镜像
8. ✅ 启动容器（5端口映射 + MQTT服务器地址 + 固件域名环境变量）
9. ✅ 执行健康检查（HTTPS API + 设备网关 双重检测）
10. ✅ 显示管理员初始密码 + MQTT服务器地址 + 固件域名信息
...
> ⚡ **版本历史**:
> **v9.0** (2026-03-14) — 部署时新增反向代理模式选择（`[Y]/[n]`格式，默认Y）；选择反代后弹出Range头说明+菜单15入口；修复Nginx配置文件搜索（`resolve_nginx_conf_interactive`：自动搜索5个常用目录并列表展示）；新增菜单15 Nginx Range头配置管理（5项子功能）
