...
###############################################################################
# ota Docker 自动部署与管理脚本
# 文件名: ota-docker-deploy.sh
...
check_port_conflicts() {
    ...
    for i in "${!PORTS[@]}"; do
        ...
        if $IN_USE; then
            ...
            # 判断是否是 ota 自身容器占用（docker-proxy 映射或容器名匹配）
            local IS_SELF=false
            if echo "$OCCUPIER" | grep -qiE "docker-proxy|docker"; then
                # docker-proxy 占用 = 可能是 ota 自身容器的端口映射
            ...
            if $IS_SELF; then
                SELF_CONFLICT=true
                echo -e "  ${YELLOW}⚡${NC} 端口 ${PORT} (${NAME}) — ota 自身容器占用（将自动释放）"
            else
                EXT_CONFLICT=true
    ...
    if $SELF_CONFLICT && ! $EXT_CONFLICT; then
        echo ""
        log_info "检测到 ota 自身容器占用端口，后续步骤会自动停止旧容器并释放端口"
    ...
    if $EXT_CONFLICT; then
        echo ""
        log_error "存在端口冲突！其他服务占用了 ota 需要的端口"
        echo ""
        echo "解决方案:"
...
auto_configure_nginx_range() {
    ...
    # 插入 Range 头配置
    sed -i '/location.*\/firmware/a\        # ota Range头透传（实现OTA进度0%→100%实时追踪）\n        proxy_set_header Range $http_range;\n        proxy_set_header If-Range $http_if_range;\n        proxy_buffering off;\n        proxy_cache off;' "$NGINX_CONF"
...
remove_nginx_range_config() {
    ...
    sed -i '/# ota Range头透传/d' "$NGINX_CONF"
...
show_range_manual_guide() {
    ...
    echo ""
    echo "    # ota Range头透传（实现OTA进度0%→100%实时追踪）"
    echo "    proxy_set_header Range \$http_range;"
    echo "    proxy_set_header If-Range \$http_if_range;"
    echo "    proxy_buffering off;"
    echo "    proxy_cache off;"
...
search_all_certs() {
    ...
    echo "操作:"
    echo "  输入编号 [1-${total_found}] 部署指定证书到 ota"
    echo "  输入 0 返回"
    echo ""
...
deploy_cert_cross_domain() {
    ...
    # 确认部署
    read -ep "是否将此证书部署到 ota? [Y/n]: " deploy_confirm
...
apply_wildcard_cert() {
    ...
    if [ $? -eq 0 ]; then
        ...
        if [ -f "$cert_path" ] && [ -f "$key_path" ]; then
            ...
            echo ""
            read -ep "是否部署此证书到 ota? [Y/n]: " deploy_confirm
            if [[ ! "$deploy_confirm" =~ ^[Nn]$ ]]; then
    ...
    case $renew_choice in
        1)
            ...
            if [ $? -eq 0 ]; then
                log_success "证书续期完成"
                echo ""
                read -ep "是否重新部署证书到 ota 并重启容器? [Y/n]: " redeploy
                if [[ ! "$redeploy" =~ ^[Nn]$ ]]; then
...
setup_cert_auto_renew() {
    ...
    echo "  每天凌晨3点检查并续期证书，续期成功后自动重启ota容器"
    echo ""
...
# v16.3: 内部函数 - 重新部署 Let's Encrypt 证书到 ota
_redeploy_letsencrypt_cert() {
    local domain="${1:-}"
...
smart_auto_renew_cert() {
    ...
    case "$ch_tool" in
        ...
        "宝塔面板"|"1Panel"|"AppNode")
            ...
            echo "  2. 添加续期后同步到 ota 的 cron 任务"
            echo ""
            echo -e "  ${BOLD}是否添加证书同步检查任务?${NC}"
            echo "  (每天检查面板证书是否更新，如有更新则自动同步到 ota)"
            echo ""
...
fix_default_nginx_cert() {
    ...
    if [ ! -f "${cert}" ] || [ ! -f "${key}" ]; then
        log_error "ota 证书文件缺失: ${cert} 或 ${key}"
...
show_commands() {
    ...
    echo "  重置密码:     sudo ./ota-docker-deploy.sh (菜单选项4)"
    echo ""
}
...
backup_data() {
    ...
    local BACKUP_FILE="${BACKUP_BASE_DIR}/ota-data-${TIMESTAMP}.tar.gz"
    ...
    log_info "开始备份..."
    if sudo tar -czf ${BACKUP_FILE} -C /opt ota/ 2>&1; then
...
restore_data() {
    ...
    if [ ! -d "${BACKUP_BASE_DIR}" ]; then
        ...
        local BACKUP_FILES=($(ls -t ${BACKUP_BASE_DIR}/ota-data-*.tar.gz 2>/dev/null))
